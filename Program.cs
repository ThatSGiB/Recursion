...
class Program
{
    ...
    start:
        ...
        //Calls the recursion method, sending in the completed sorted array
        Recursion(ref aSorted);

        //Removed the for loop that printed the array, instead having it be printed through the Recursion method
    ...
    //Accepts an array of double, then prints out the contents of the array, calling itself and removing the first value of the array each time
    static void Recursion(ref double[] sArray)
    {
        //runs through the contents of the array and prints each one
        foreach(var s in sArray)
        {
            Console.Write(s);
        }
        //creates a new line to seperate each instance of the array
        Console.WriteLine();

        //determines if the array still has content other than one value
        if (sArray.Length > 1)
        {
            //calling the RemoveArrayValue method to remove the first value of the array
            RemoveArrayValue(sArray[0], ref sArray);
            //calls itself again to run it, placed inside the if statement, otherwise it would run forever
            Recursion(ref sArray);
        }
    }
}